    ...
    <div className="space-y-8 w-full max-w-4xl mx-auto">
      <div className="text-center space-y-2">
        <h3 className={`text-xl font-bold ${theme.colors.text.primary}`}>
          Others Collection ({OTHERS_COMPONENTS.length} Mẫu Component)
        </h3>
        <p className={`text-sm ${theme.colors.text.secondary}`}>
          Các component tiện ích đặc biệt. Bấm vào tiêu đề chữ bên dưới mỗi card để tới trang chi tiết.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
        {OTHERS_COMPONENTS.map((item) => (
          <div
            key={item.id}
